...
while not end:
    ...
    if func == "1":
        ...
        f4 = input("Voulez-vous savoir quel est/sont le(s) nom(s) du (des) président(s) qui a (ont) parlé du mot de votre choix ainsi que celui qui l’a répété le plus de  fois ? (y) or (n) : ")
        f4 = f4.lower()
        if f4 == "y":
            ...
            if Max == 0:
                ...
                if len(M2) > 1:
                    ...
                    print(f"{M2[0]} est le seul a avoir prononcé le mot: {word_search}, {M2[0]} l'a prononcé {Max} fois.")
